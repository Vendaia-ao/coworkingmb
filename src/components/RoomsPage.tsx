...
import { Check, Users, Presentation, GraduationCap, Lock, Wifi, Coffee, ArrowRight, Info } from 'lucide-react';
  ...
  const roomTypes = [
    {
      ...
      price: "5.000",
      period: "Kz (até às 12H)",
      priceAlt: "10.500 Kz/hora após as 12H",
      icon: Presentation,
      features: [
        "Disponível de segunda a sábado",
        "Lotação até 8 pessoas",
        "Smart TV & HDMI",
        "Quadro Branco",
        "Internet de Alta Velocidade",
        "Coffee Break disponível"
      ],
      highlight: false,
      note: "Período matinal: até às 12H. Após as 12H: 10.500 Kz/hora."
    },
    {
      ...
      icon: GraduationCap,
      features: [
        "Espaço amplo para formações",
        "Projetor ou Smart TV",
        "Quadro Branco",
        "Internet de Alta Velocidade",
        "Climatização",
        "Coffee Break disponível"
    ...
    },
    {
      ...
      icon: Lock,
      features: [
        "Escritório privado para 1 pessoa",
        "Mobiliário completo",
        "Internet de Alta Velocidade",
      ...
      highlight: false,
      note: "Adição de pessoas na sala privada mediante negociação."
    }
  ];
    ...
    <div className="bg-gray-50 min-h-screen animate-fade-in">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="/rooms.jpg" alt="Salas de reunião e formação em Luanda - Coworking MB" className="w-full h-full object-cover" />
          ...
          <div className="bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col lg:flex-row">
            <div className="w-full lg:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-texture-noise">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-10">
                <div className="flex items-start gap-3">
                  <div className="bg-gray-50 p-2 rounded-lg text-gold-dark"><Wifi size={24} /></div>
                  <div>
                    <h4 className="font-bold text-brand-dark">Internet de Alta Velocidade</h4>
                    <p className="text-sm text-gray-500">Conexão estável e rápida.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="bg-gray-50 p-2 rounded-lg text-gold-dark"><Coffee size={24} /></div>
                  <div>
                    <h4 className="font-bold text-brand-dark">Amenidades Incluídas</h4>
                    <p className="text-sm text-gray-500">Copa, água mineral e café.</p>
                  </div>
                </div>
              </div>
              <div className="mt-10">
                <Link to="/contacto" className="inline-flex items-center gap-2 text-brand-dark font-bold border-b-2 border-gold pb-1 hover:text-gold transition-colors uppercase tracking-widest text-sm">
            ...
            <div className="w-full lg:w-1/2 relative min-h-[400px]">
              ...
              <div className="absolute bottom-8 left-8 right-8 glass-panel p-6 rounded-lg border-l-4 border-gold">
                <p className="text-brand-dark font-serif italic text-lg">"Ambientes que inspiram produtividade e sucesso."</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-brand-dark text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-serif font-bold mb-4">Tipos de Salas & Preços</h2>
            <div className="w-24 h-1 gold-gradient-bg mx-auto rounded-full"></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            {roomTypes.map((room, index) => (
              <div key={index} className={`relative rounded-xl overflow-hidden transition-all duration-300 flex flex-col h-full ${
                room.highlight ? 'transform md:-translate-y-6 shadow-2xl border-2 border-gold bg-white z-10' : 'bg-white shadow-xl border border-gray-100 mt-0 md:mt-4'
              }`}>
                {room.highlight && <div className="absolute top-0 inset-x-0 h-1 gold-gradient-bg"></div>}
                {room.highlight && (
                  <div className="absolute top-4 right-4">
                    <span className="bg-gold text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">Popular</span>
                  </div>
                )}
                <div className="p-8 flex flex-col h-full">
                  <div className="mb-6 text-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 mx-auto ${
                      room.highlight ? 'gold-gradient-bg text-white shadow-lg' : 'bg-gray-100 text-brand-dark'
                    }`}>
                      <room.icon size={32} strokeWidth={1.5} />
                    </div>
                    <h3 className="text-2xl font-serif font-bold text-brand-dark mb-2">{room.name}</h3>
                    <div className="flex flex-col items-center">
                      <span className={`text-3xl font-bold tracking-tight ${room.highlight ? 'text-gold-dark' : 'text-gray-900'}`}>{room.price}</span>
                      <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">{room.period}</span>
                      {room.priceAlt && <span className="text-xs text-gold-dark font-semibold mt-1">{room.priceAlt}</span>}
                    </div>
                  </div>
                    ...
                    {room.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start">
                        <span className={`flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center mr-3 mt-1 ${
                          room.highlight ? 'bg-gold/20 text-gold-dark' : 'bg-gray-100 text-gray-400'
                  ...
                  {room.note && (
                    <div className="flex items-start gap-2 mb-6 bg-gray-50 p-3 rounded-lg">
                      <Info size={14} className="text-gold-dark mt-0.5 flex-shrink-0" />
                  ...
                  <button onClick={() => document.getElementById('booking-section')?.scrollIntoView({behavior: 'smooth'})}
                    className={`w-full py-4 rounded-sm font-bold uppercase tracking-widest text-sm transition-all duration-300 ${
                      room.highlight ? 'gold-gradient-bg text-white shadow-lg hover:shadow-gold/50' : 'bg-brand-dark text-white hover:bg-black'
                    }`}>
