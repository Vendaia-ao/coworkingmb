...
const ICONS: Record<string, any> = { 'Bronze': Users, 'Prata': Wifi, 'Diamante': Clock };
    ...
    <div className="bg-gray-50 min-h-screen animate-fade-in">
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="/desks.jpg" alt="Secretárias Mulato Business" className="w-full h-full object-cover" />
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col lg:flex-row">
            <div className="w-full lg:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-texture-noise">
              <h2 className="text-3xl font-serif font-bold text-brand-dark mb-8 relative">Flexibilidade Total<div className="absolute -bottom-2 left-0 w-16 h-1 gold-gradient-bg"></div></h2>
              <div className="space-y-6 text-gray-600 leading-relaxed text-lg">
                <p>Opções de secretárias <strong className="text-brand-dark">partilhadas ou dedicadas</strong> para quem busca um espaço profissional e produtivo.</p>
                <p>Ideal para freelancers, empreendedores e profissionais que precisam de um ambiente de trabalho inspirador sem compromissos de longo prazo.</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-10">
                <div className="flex items-start gap-3"><div className="bg-gray-50 p-2 rounded-lg text-gold-dark"><Wifi size={24} /></div><div><h4 className="font-bold text-brand-dark">Internet Rápida</h4><p className="text-sm text-gray-500">Conexão de alta velocidade incluída.</p></div></div>
                <div className="flex items-start gap-3"><div className="bg-gray-50 p-2 rounded-lg text-gold-dark"><Coffee size={24} /></div><div><h4 className="font-bold text-brand-dark">Copa Equipada</h4><p className="text-sm text-gray-500">Espaço para refeições e café.</p></div></div>
              </div>
            ...
            <div className="w-full lg:w-1/2 relative min-h-[400px]">
              <img src="/secretarias.png" alt="Secretárias Mulato Business" className="absolute inset-0 w-full h-full object-cover" />
      ...
      <section className="py-20 bg-brand-dark text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16"><h2 className="text-3xl font-serif font-bold mb-4">Nossos Planos</h2><div className="w-24 h-1 gold-gradient-bg mx-auto rounded-full"></div></div>
          {loading ? <div className="grid grid-cols-1 md:grid-cols-3 gap-8">{[1,2,3].map(i => <Skeleton key={i} className="h-96 rounded-xl" />)}</div> : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {plans.map((plan) => {
                const Icon = ICONS[plan.nome] || Users;
                return (
                  <div key={plan.id} className={`relative rounded-xl overflow-hidden transition-all duration-300 group ${plan.destaque ? 'transform md:-translate-y-4 shadow-2xl border-2 border-gold bg-white' : 'bg-white shadow-xl hover:-translate-y-2 border border-gray-100'}`}>
                    ...
                    {plan.destaque && <div className="absolute top-4 right-4"><span className="bg-gold text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">Popular</span></div>}
                    <div className="p-8 flex flex-col h-full">
                      <div className="mb-6 text-center">
                        <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 mx-auto ${plan.destaque ? 'gold-gradient-bg text-white shadow-lg' : 'bg-gray-100 text-brand-dark'}`}><Icon size={28} strokeWidth={1.5} /></div>
                        <h3 className="text-2xl font-serif font-bold text-brand-dark mb-2">{plan.nome}</h3>
                        <div className="flex items-baseline justify-center gap-1">
                          <span className={`text-3xl font-bold tracking-tight ${plan.destaque ? 'text-gold-dark' : 'text-gray-900'}`}>{plan.preco}</span>
                      ...
                      <ul className="space-y-3 mb-8 flex-grow">
                        {plan.features.map((f, idx) => (
                          <li key={idx} className="flex items-start">
                            <span className={`flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center mr-3 mt-1 ${plan.destaque ? 'bg-gold/20 text-gold-dark' : 'bg-gray-100 text-gray-400'}`}><Check size={10} strokeWidth={3} /></span>
