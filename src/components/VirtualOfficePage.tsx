  ...
  const plans = [
    {
      ...
      period: "Kz / mês",
      icon: MapPin,
      features: ["Endereço Fiscal de Prestígio"],
      highlight: false,
      promo: null
    },
    {
      ...
      period: "Kz / mês",
      icon: User,
      features: ["Endereço Fiscal de Prestígio", "Rececionista partilhada"],
      highlight: false,
      promo: null
    },
    {
      ...
      highlight: true,
      promo: "🎁 Promoção: Ao subscrever o pacote Escritório Virtual Plus, ganha +5H na Sala de Reunião, totalizando 10H mensais!"
    }
  ];
    ...
    <div className="bg-gray-50 min-h-screen animate-fade-in">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="/virtualroom.jpg" alt="Escritório Virtual em Luanda - Coworking MB" className="w-full h-full object-cover" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan, index) => (
              <div key={index} className={`relative rounded-xl overflow-hidden transition-all duration-300 group ${
                plan.highlight ? 'transform md:-translate-y-4 shadow-2xl border-2 border-gold bg-white' : 'bg-white shadow-xl hover:-translate-y-2 border border-gray-100'
              }`}>
                {plan.highlight && <div className="absolute top-0 inset-x-0 h-1 gold-gradient-bg"></div>}
                {plan.highlight && (
                  <div className="absolute top-4 right-4">
                    <span className="bg-gold text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">Recomendado</span>
                  </div>
                )}
                <div className="p-8 md:p-10 flex flex-col h-full">
                  <div className="mb-6">
                    ...
                    }`}><plan.icon size={28} strokeWidth={1.5} /></div>
                    <h3 className="text-2xl font-serif font-bold text-brand-dark mb-2">{plan.name}</h3>
                    <div className="flex items-baseline gap-1">
                      <span className={`text-3xl md:text-4xl font-bold tracking-tight ${plan.highlight ? 'text-gold-dark' : 'text-gray-900'}`}>{plan.price}</span>
                  ...
                  <button onClick={() => document.getElementById('booking-section')?.scrollIntoView({behavior: 'smooth'})}
                    className={`w-full py-4 rounded-sm font-bold uppercase tracking-widest text-sm transition-all duration-300 flex items-center justify-center gap-2 ${
                      plan.highlight ? 'gold-gradient-bg text-white shadow-lg hover:shadow-gold/50' : 'bg-brand-dark text-white hover:bg-black'
                    }`}>
                    Solicitar Informação <ArrowRight size={16} />
                  </button>
                </div>
