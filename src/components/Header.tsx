...
export const Header: React.FC = () => {
  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isScrolled ? 'glass-panel shadow-md' : 'bg-transparent'
        }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
          ...
          <div className="flex-shrink-0 flex items-center cursor-pointer">
            <Link to="/" className="block">
              ...
              >
                <img
                  src="/logotipo.png"
                  alt="Coworking MB - Mulato Business em Angola"
              ...
              >
                {link.dropdown ? (
                  <button
                    ...
                    className={`text-sm font-medium tracking-wide transition-colors duration-200 flex items-center gap-1 cursor-pointer ${isScrolled ? 'text-gray-700 hover:text-gold' : 'text-gray-200 hover:text-white'
                      }`}
                  >
                    {link.label}
                    <motion.span
                  ...
                  <button
                    onClick={() => handleLinkClick(link.href)}
                    className={`text-sm font-medium tracking-wide transition-colors duration-200 cursor-pointer ${isScrolled ? 'text-gray-700 hover:text-gold' : 'text-gray-200 hover:text-white'
                      }`}
                  >
                    {link.label}
                  </button>
                        ...
                        {link.dropdown.map((dropItem, idx) => (
                          ...
                          >
                            <Link
                              ...
                              className={`block px-4 py-2 text-sm text-gray-700 hover:bg-gold-light/20 hover:text-gold-dark cursor-pointer ${isActiveRoute(dropItem.href) ? 'bg-gold-light/20 text-gold-dark' : ''
                                }`}
                            >
                              {dropItem.label}
                            </Link>
                ...
                >
                  {link.dropdown ? (
                      ...
                      <div className="pl-6 space-y-1">
                        {link.dropdown.map((drop) => (
                          <Link
                            ...
                            className={`block px-3 py-2 text-sm font-medium text-gray-500 hover:text-gold-dark cursor-pointer ${isActiveRoute(drop.href) ? 'text-gold-dark' : ''
                              }`}
                          >
                            {drop.label}
                          </Link>
                    ...
                    <Link
                      to={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`block px-3 py-3 text-base font-medium text-gray-800 hover:text-gold-dark hover:bg-gray-50 rounded-md cursor-pointer ${isActiveRoute(link.href) ? 'text-gold-dark' : ''
                        }`}
                    >
                      {link.label}
                    </Link>
